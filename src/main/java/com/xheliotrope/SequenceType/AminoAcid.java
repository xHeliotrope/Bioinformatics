...
public class AminoAcid extends Sequence {
    ...
    public AminoAcid(String aaString, String fullName){
        this.aaString = aaString;
        this.fullName = fullName;
    }

    private String aaString, fullName;
    ...
    public AminoAcid dnaToAmino(DNA dna){
        try {
            DNA cutSequence = new DNA(cutUpStreamSequence(dna.getSequenceString()), dna.getFullName() + " from DNA");
            ...
            for (int i = 0; i < dna.getSequenceString().length(); i += 3) {
                ...
                for (String terminator : stopCodon) {
                    if (codon.equals(terminator)) {
                        return new AminoAcid(newAminoSequence, " from dna");
                    }
                }
